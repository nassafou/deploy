...
MYSQL_PWD=gnulinux mysql -u bob -sN -e "SELECT table_schema 'Data Base Name', round(sum( data_length + index_length) / 1024 / 1024, 2) 'Data Base Size in MB' FROM information_schema.TABLES where table_schema not in ('mysql', 'performance_schema', 'information_schema') GROUP BY table_schema ;"| tr "\t" ";" | awk -v date_jour="$date_jour" -v host="$HOSTNAME" '{print date_jour";"host";db_taille;"$0}' >>/var/log/datas.csv

# nombre de tables

MYSQL_PWD=gnulinux mysql -u bob -sN -e "select table_schema, count(*) from information_schema.TABLES where table_schema not in ('mysql', 'performance_schema', 'information_schema')group by TABLE_SCHEMA;"| tr "\t" ";" |  awk -v date_jour="$date_jour" -v host="$HOSTNAME" '{print date_jour";"host";db_tables;"$0}'>>/var/log/datas.csv

# nombre lignes par table

MYSQL_PWD=gnulinux mysql -u bob  -sN -e "select table_schema,table_name,TABLE_ROWS from information_schema.TABLES where table_schema not in ('mysql', 'performance_schema', 'information_schema') group by TABLE_SCHEMA, TABLE_NAME;"| tr "\t" ";" |  awk -v date_jour="$date_jour" -v host="$HOSTNAME" '{print date_jour";"host";db_lines;"$0}'>>/var/log/datas.csv
...
for database in $(MYSQL_PWD=gnulinux mysql -u bob  -sN -e "select table_schema from information_schema.TABLES where table_schema not in ('mysql', 'performance_schema', 'information_schema') group by TABLE_SCHEMA;");do
...
curl -i -X PUT 192.168.205.10:5000/datas?machine=$(hostname)\&type=mysql --upload-file /var/log/datas.csv
